Webruntime.define('lwc/toDoList', ['lwc', 'lightning/configProvider'], function (lwc, configProvider) { 'use strict';
    ...
    **/
    ...
    if (_implicitStylesheets$3) {
      tmpl$9.stylesheets.push.apply(tmpl$9.stylesheets, _implicitStylesheets$3);
    }
    ...
    * */
    ...
    function tmpl$p($api, $cmp, $slotset, $ctx) {
      ...
      return [api_custom_element("lightning-card", _lightningCard, {
        ...
        key: 16
      }, api_flatten([$cmp.isEdit ? api_iterator($cmp.todoTask, function (todoItems) {
        return api_element("li", {
          classMap: {
            "slds-item": true
          },
          key: api_key(7, todoItems.id)
        }, [api_custom_element("lightning-layout", _lightningLayout, {
          props: {
          ...
          key: 1,
          on: {
            "change": _m0 || ($ctx._m0 = api_bind($cmp.innerInput))
          }
        }, [])]), api_element("div", {
          classMap: {
          ...
          key: 3,
          on: {
            "click": _m1 || ($ctx._m1 = api_bind($cmp.saveTask))
          }
        }, [])])])])]);
      }) : [], api_iterator($cmp.todoTasks, function (todoItem) {
        return api_element("li", {
          classMap: {
            "slds-item": true
          },
          key: api_key(15, todoItem.id)
        }, [api_custom_element("lightning-layout", _lightningLayout, {
          props: {
            "verticalAlign": "center",
            "horizontalAlign": "spread"
          ...
          key: 8
        }, [api_dynamic(todoItem.name)]), api_element("div", {
          classMap: {
            "slds-button-group": true
          },
          attrs: {
          ...
          key: 9,
          on: {
            "click": _m2 || ($ctx._m2 = api_bind($cmp.deleteTask))
          }
        }, [])]), api_custom_element("lightning-layout-item", _lightningLayoutItem, {
          classMap: {
          ...
          key: 11,
          on: {
            "click": _m3 || ($ctx._m3 = api_bind($cmp.editTask))
          }
        }, [])])])])]);
      })]))])]), api_element("p", {
        attrs: {
        ...
        key: 19,
        on: {
          "change": _m4 || ($ctx._m4 = api_bind($cmp.updateTaskValue))
        }
      }, [])]), api_custom_element("lightning-layout-item", _lightningLayoutItem, {
        props: {
          "flexibility": "auto",
        ...
        key: 21,
        on: {
          "click": _m5 || ($ctx._m5 = api_bind($cmp.addTaskItem))
        }
      }, [])])])])])];
    }
    ...
    tmpl$p.stylesheetTokens = {
      hostAttribute: "lwc-toDoList_toDoList-host",
      shadowAttribute: "lwc-toDoList_toDoList"
    };

    class ToDoList extends lwc.LightningElement {
      constructor(...args) {
        super(...args);
        this.newTask = '';
        this.isEdit = void 0;
      ...
      updateTaskValue(event) {
        ...
        this.newTask = event.target.value; //console.log(this.newTask);

        var id = event.target.id; // console.log(id);
      }
      ...
      deleteTask(event) {
        ...
        for (let i = 0; i < todoTaskk.length; i++) {
          if (idToDelete === todoTaskk[i].id) toDoTaskIndex = i;
        }
      ...
      editTask(event) {
        var idd = event.target.value; //var na=event.target.name;

        let todoTa = this.todoTask; //console.log(idd);

        var todoTaskk = this.todoTasks; //console.log(JSON.stringify(todoTaskk));

        let idToInsert;

        for (let i = 0; i < todoTaskk.length; i++) {
          if (idd === todoTaskk[i].id) //idtoPop= (todoTaskk[i]);
            idToInsert = todoTaskk[i];
          {
            todoTaskk.pop(idToInsert); //todoTaskk[i]=null;

            console.log(JSON.stringify(todoTaskk)); //var elementNo=todoTaskk.length;

            todoTa.push({
              id: this.todoTa.length + 1,
              name: ''
            });
            console.log(JSON.stringify(todoTa));
            this.isEdit = 'true'; //console.log(JSON.stringify(this.todoTask));
          }
        }
      }
      ...
      saveTask(event) {
        var saveId = event.target.value; // console.log(saveId);
    ...
    lwc.registerDecorators(ToDoList, {
      track: {
        isEdit: 1,
        todoTasks: 1,
        todoTask: 1
      },
      fields: ["newTask", "todoIte"]
    });

    var toDoList = lwc.registerComponent(ToDoList, {
      tmpl: _tmpl$o
    });
